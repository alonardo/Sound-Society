/**
 * Rhymes and Reason - Music Sociology Dashboard
...
const DECADE_MS = 4000; // 4 seconds per decade at 1x speed

function initTimelineSection() {
    // Populate genre dropdown
    const genreSelect = document.getElementById('timeline-genre');
    const genres = dashboardData.metadata.genres || [];
    const byGenre = dashboardData.aggregations.by_genre;

    // Sort genres by song count
    const sortedGenres = genres
        .map(g => ({ code: g, data: byGenre[g] }))
        .sort((a, b) => (b.data?.count || 0) - (a.data?.count || 0));

    genreSelect.innerHTML = '<option value="">Select a genre...</option>';
    sortedGenres.forEach(({ code, data }) => {
        genreSelect.innerHTML += `<option value="${code}">${data?.label || code}</option>`;
    });

    // Event listeners
    genreSelect.addEventListener('change', (e) => selectTimelineGenre(e.target.value));
    ...
    // Playback controls
    document.getElementById('tl-play').addEventListener('click', togglePlayPause);
    document.getElementById('tl-prev').addEventListener('click', () => goToDecade(0));
    document.getElementById('tl-next').addEventListener('click', () => goToDecade(timelineState.availableDecades.length - 1));
    document.getElementById('tl-back').addEventListener('click', prevDecade);
    document.getElementById('tl-forward').addEventListener('click', nextDecade);

    // Initialize scrubber with all decades (grayed out until genre selected)
    renderTimelineScrubber();
}

function selectTimelineGenre(genre) {
    if (!genre) {
        timelineState.genre = null;
        timelineState.availableDecades = [];
        document.getElementById('timeline-content').style.display = 'none';
        document.getElementById('tl-decade-label').textContent = 'Select a genre to begin';
        renderTimelineScrubber();
        return;
    }

    pauseTimeline();
    timelineState.genre = genre;

    // Determine available decades for this genre
    const genreDecadeData = dashboardData.word_frequencies?.by_genre_decade || {};
    timelineState.availableDecades = timelineState.decades.filter(decade => {
        const key = `${genre}_${decade}`;
        return genreDecadeData[key] && genreDecadeData[key].length > 0;
    });

    if (timelineState.availableDecades.length === 0) {
        document.getElementById('timeline-content').style.display = 'none';
        document.getElementById('tl-decade-label').textContent = 'No data for this genre';
        return;
    }
    ...
    // Show content and go to first decade
    document.getElementById('timeline-content').style.display = 'block';
    timelineState.decadeIndex = 0;
    renderTimelineScrubber();
    goToDecade(0);
}
...
function initTimelineCharts() {
    ...
    if (timelineState.charts.wordFreq) {
        timelineState.charts.wordFreq.destroy();
    }
    if (timelineState.charts.sentiment) {
        timelineState.charts.sentiment.destroy();
    }

    // Word frequency chart
    const wordCtx = document.getElementById('tl-word-chart').getContext('2d');
    timelineState.charts.wordFreq = new Chart(wordCtx, {
        type: 'bar',
        data: {
            labels: [],
            datasets: [{
                label: 'Word Count',
                data: [],
                backgroundColor: GENRE_COLORS[timelineState.genre] || '#f97316',
                borderWidth: 0,
                borderRadius: 4
            }]
        },
    ...
    const sentCtx = document.getElementById('tl-sentiment-chart').getContext('2d');
    const metricsData = dashboardData.timeline?.metrics_by_genre_year?.[timelineState.genre] || {};
    const years = Object.keys(metricsData).sort();
    const sentiments = years.map(y => metricsData[y]?.avg_sentiment);

    timelineState.charts.sentiment = new Chart(sentCtx, {
        type: 'line',
        data: {
            labels: years,
            datasets: [{
                label: 'Sentiment',
                data: sentiments,
                borderColor: GENRE_COLORS[timelineState.genre] || '#f97316',
                backgroundColor: `${GENRE_COLORS[timelineState.genre] || '#f97316'}20`,
                borderWidth: 2,
                fill: true,
                tension: 0.3,
                pointRadius: 1
...
function renderTimelineScrubber() {
    ...
    const { decades, availableDecades, decadeIndex, genre } = timelineState;

    container.innerHTML = decades.map((decade, idx) => {
        const isAvailable = availableDecades.includes(decade);
        const availableIdx = availableDecades.indexOf(decade);
        const isPassed = genre && isAvailable && availableIdx < decadeIndex;
        const isActive = genre && isAvailable && availableIdx === decadeIndex;

        let classes = 'decade-marker';
        if (!isAvailable && genre) classes += ' unavailable';
        ...
        return `
            <div class="${classes}"
                 data-decade="${decade}"
                 data-available-idx="${availableIdx}"
                 onclick="handleScrubberClick('${decade}', ${availableIdx})">
                ${decade}
            </div>
        `;
    }).join('');
    ...
    // Update progress bar
    if (genre && availableDecades.length > 1) {
        const progress = (decadeIndex / (availableDecades.length - 1)) * 100;
...
function handleScrubberClick(decade, availableIdx) {
    if (availableIdx >= 0 && timelineState.availableDecades.includes(decade)) {
        goToDecade(availableIdx);
    }
}

function goToDecade(index) {
    if (index < 0 || index >= timelineState.availableDecades.length) return;

    timelineState.decadeIndex = index;
    const decade = timelineState.availableDecades[index];
    const genre = timelineState.genre;

    // Update label
    document.getElementById('tl-decade-label').textContent = decade;

    // Update scrubber
    renderTimelineScrubber();

    // Update all visualizations
    updateTimelineWordChart(genre, decade);
    updateTimelineTfidf(genre, decade);
    updateTimelineTopSongs(genre, decade);
    updateTimelineArtists(genre, decade);
    updateTimelineSentimentHighlight(decade);
    updateTimelineEvents(decade);
}

function updateTimelineWordChart(genre, decade) {
    const key = `${genre}_${decade}`;
    const wordData = dashboardData.word_frequencies?.by_genre_decade?.[key] || [];
    const topWords = wordData.slice(0, 15);

    const chart = timelineState.charts.wordFreq;
    chart.data.labels = topWords.map(w => w.word);
    chart.data.datasets[0].data = topWords.map(w => w.count);
    chart.data.datasets[0].backgroundColor = GENRE_COLORS[genre] || '#f97316';
    chart.update();
}

function updateTimelineTfidf(genre, decade) {
    const container = document.getElementById('tl-tfidf');
    const key = `${genre}_${decade}`;
    const tfidfData = dashboardData.tfidf?.by_genre_decade?.[key] || [];

    if (tfidfData.length === 0) {
        container.innerHTML = '<p style="color: var(--text-muted); padding: 1rem;">No distinctive words data for this period.</p>';
        return;
    }

    // Assign sizes based on TF-IDF score ranking
    container.innerHTML = tfidfData.map((word, idx) => {
        let sizeClass = '';
        if (idx < 3) sizeClass = 'large';
        else if (idx < 8) sizeClass = 'medium';

        return `<span class="tfidf-tag ${sizeClass} entering" style="animation-delay: ${idx * 50}ms">${word.word}</span>`;
    }).join('');
}

function updateTimelineTopSongs(genre, decade) {
    const container = document.getElementById('tl-songs');
    const key = `${genre}_${decade}`;
    const songs = dashboardData.timeline?.top_songs_by_genre_decade?.[key] || [];

    if (songs.length === 0) {
        container.innerHTML = '<p style="color: var(--text-muted); padding: 1rem;">No song data available.</p>';
        return;
    }

    container.innerHTML = songs.map(song => `
        <div class="tl-song-item">
            <span class="tl-song-rank">#${song.rank}</span>
            <div class="tl-song-info">
                <div class="tl-song-title">${escapeHtml(song.title)}</div>
                <div class="tl-song-artist">${escapeHtml(song.artist)}</div>
            </div>
            <span class="tl-song-year">${song.year}</span>
        </div>
    `).join('');
}

function updateTimelineArtists(genre, decade) {
    const container = document.getElementById('tl-artists');
    const key = `${genre}_${decade}`;
    const artists = dashboardData.timeline?.top_artists_by_genre_decade?.[key] || [];

    if (artists.length === 0) {
        container.innerHTML = '<p style="color: var(--text-muted); padding: 1rem;">No artist data available.</p>';
        return;
    }

    container.innerHTML = artists.map(artist => `
        <div class="tl-artist-item">
            <span class="tl-artist-name">${escapeHtml(artist.artist)}</span>
            <span class="tl-artist-count">${artist.song_count} song${artist.song_count > 1 ? 's' : ''}</span>
        </div>
    `).join('');
}

function updateTimelineSentimentHighlight(decade) {
    const chart = timelineState.charts.sentiment;
    if (!chart) return;
    ...
    // Update annotation to highlight current decade
    chart.options.plugins.annotation = {
        annotations: {
            box1: {
                type: 'box',
                xMin: String(decadeStart),
                xMax: String(decadeEnd),
                backgroundColor: 'rgba(249, 115, 22, 0.1)',
...
function updateTimelineEvents(decade) {
    const container = document.getElementById('tl-events-list');

    if (!eventsData?.events || !eventsData?.categories) {
        container.innerHTML = '<p style="color: var(--text-muted);">No events data available.</p>';
        return;
    }
    ...
    // Filter events for this decade
    const decadeEvents = eventsData.events.filter(event => {
        return event.year >= decadeStart && event.year <= decadeEnd;
    });

    if (decadeEvents.length === 0) {
        container.innerHTML = '<p style="color: var(--text-muted);">No major events recorded for this decade.</p>';
        return;
    }

    container.innerHTML = decadeEvents.map(event => {
        const cat = eventsData.categories[event.category];
        const color = cat?.color || '#6b7280';

        return `
            <div class="tl-event-item" style="--event-color: ${color}">
                <span class="tl-event-dot" style="background: ${color}"></span>
...
function playTimeline() {
    if (timelineState.isPlaying || !timelineState.genre) return;
    if (timelineState.availableDecades.length === 0) return;

    // If at the end, restart from beginning
    if (timelineState.decadeIndex >= timelineState.availableDecades.length - 1) {
        timelineState.decadeIndex = 0;
        goToDecade(0);
    }
    ...
    timelineState.interval = setInterval(() => {
        if (timelineState.decadeIndex < timelineState.availableDecades.length - 1) {
            nextDecade();
        } else {
            pauseTimeline();
        }
    }, DECADE_MS / timelineState.speed);
}
...
function nextDecade() {
    if (timelineState.decadeIndex < timelineState.availableDecades.length - 1) {
        goToDecade(timelineState.decadeIndex + 1);
    }
}

function prevDecade() {
    if (timelineState.decadeIndex > 0) {
        goToDecade(timelineState.decadeIndex - 1);
    }
}
